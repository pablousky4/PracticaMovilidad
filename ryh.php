...
<?php
   ...
   $inputLugar="<input class=\"input\" type=\"text\" name=\"lugar\" placeholder=\"C/Valeras,30\">";
   $inputSalida="<input class=\"input\" type=\"date\" name=\"salida\" >";
   $inputLlegada="<input class=\"input\" type=\"date\" name=\"llegada\" >";
   ...
   echo $inputMatricula;
   echo"<br> <p>Introduce Lugar: </p>";
   echo $inputLugar;
   echo "<br> <p>Introduce Fecha Llegada: </p>";
   echo $inputLlegada;
   echo "<br> <p>Introduce Fecha Salida: </p>";
   echo $inputSalida;
   ...
   echo "</form>";
    ...
    if (isset($_POST['botonSI'])){ //Si hemos pulsado el boton
        ...
        if(preg_match($patronMatricula, $matricula)){ //cpmprueba la matricula
            ...
            $fechaSalida = DateTime::createFromFormat('Y-m-d', $salida);
            $fechaLlegada = DateTime::createFromFormat('Y-m-d', $llegada);
            ...
            if(is_uploaded_file ($_FILES['fichero']['tmp_name']) && $_FILES['fichero']['type']=="application/pdf") {
                if(!empty($lugar) || !empty($salida) || !empty($llegada) || $fechaLlegada > $hoy || $fechaLlegada > $fechaSalida){
                        if($ficheroEmt = fopen("residentesYHoteles.txt","a+")){  //abre el fichero y escribe
                            $concatenado =  "\n". $matricula ." ".$lugar." ".$fechaLlegada->format('Y/m/d')." ".$fechaSalida->format('Y/m/d');
