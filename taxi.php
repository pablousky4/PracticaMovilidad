...
<?php
   ...
   //////EMT
   $emt="taxis.txt";
   $inputNombre="<input class=\"input\" type=\"text\" name=\"nombre\" placeholder=\"Carmen\">";
   $inputFichero="<input class=\"input\" type=\"file\" name=\"fichero\">";
   ...
   echo $inputMatricula;
   echo"<br> <p>Introduce nombre del chófer: </p>";
   echo $inputNombre;
        ...
        if(preg_match($patronMatricula, $matricula)){ //cpmprueba la matricula
            ...
            if(!empty($nombre)){
                ...
                if(is_uploaded_file ($_FILES['fichero']['tmp_name']) && $_FILES['fichero']['type']=="application/pdf") {
                    if($ficheroEmt = fopen("taxis.txt","a+")){  //abre el fichero y escribe
