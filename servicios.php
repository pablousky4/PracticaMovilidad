...
<?php
   ...
   $inputvehiculo="<input class=\"input\" type=\"text\" name=\"vehiculo\" placeholder=\"Ambulancia\">";
   $inputFichero="<input class=\"input\" type=\"file\" name=\"fichero\" value=\"fichero\">";
 

   echo"<form id=\"emt\" action=\"". htmlspecialchars($_SERVER["PHP_SELF"])."\" method=\"POST\">";
   echo $header;
   ...
   echo "</form>";
    ...
    if (isset($_POST['botonSI'])){ //Si hemos pulsado el boton
        ...
        if(preg_match($patronMatricula, $matricula)){ //cpmprueba la matricula
            ...
            if(!empty($vehiuclo) || !empty($fichero)){
                if (!in_array($ext, $permitido)) {
                    if($ficheroEmt = fopen("servicios.txt","a+")){  //abre el fichero y escribe
